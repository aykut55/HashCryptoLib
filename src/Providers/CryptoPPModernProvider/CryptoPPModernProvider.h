#ifndef CRYPTOAPI_PROVIDERS_CRYPTOPP_MODERN_PROVIDER_H
#define CRYPTOAPI_PROVIDERS_CRYPTOPP_MODERN_PROVIDER_H
...
class CCryptoPPModernProvider : public IAeadCipher, public IKeyDerivation, public IRandomSource
{
public:
    virtual ~CCryptoPPModernProvider();
             CCryptoPPModernProvider();
...
private:
    ...
    // cryptopp-modern types (GCM<AES>::Encryption/Decryption) are kept out of this header so
    // callers never need cryptopp-modern's own headers or include path; only
    // CryptoPPModernProvider.cpp does.
    struct Impl;
