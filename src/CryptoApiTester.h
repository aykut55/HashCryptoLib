...
public:
    ...
    // real Ed25519/Cv25519 identity ("--quick-generate-key ... future-default") -- same SKIPPED
    // (not FAILED) conventions when gpg.exe is missing or when key generation doesn't produce a
    // usable secret key in this environment (piped/_popen gpg-agent quirk, see
    // RunPgpGnuPgMultiRecipientInteropTest). This engine (an ordinary RSA sender) imports BOTH real
    // gpg public keys (ImportPeerPublicKey for the RSA one, ImportAdditionalRecipientPublicKey for
    // the Ed25519 one) and EncryptStringArmored's ONCE; the resulting single ciphertext is handed to
    // EACH real gpg identity's own homedir in turn ("gpg --decrypt") and both must independently
